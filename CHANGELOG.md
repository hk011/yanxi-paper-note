...
- **QwenPaw Skill 集成**（`integrations/qwenpaw-skill/`）：Agent 按研析笔记生成方法解读论文，无需启动 Web 后端；PaddleOCR（首选）/ Kreuzberg（备选）PDF 解析，三阶段笔记生成，交付前 PDF 自检（正文乱码、原图整页截图/裁切不全等）

## [0.0.10] - 2026-07-04

### Added

- 论文问答附带完整 `parsed.md` 原文：多轮对话仅在最新一轮附带，避免上下文重复膨胀
- DeepSeek 模型上下文圆环上限调整为 1M tokens，切换模型时自动更新
- 文件夹内上传 PDF 时自动归入当前文件夹
- 论文多文件夹 `sort_order` 排序，封面配色按主文件夹（排序第一）取色

### Changed

- 信息图配图统一使用商汤 Nova，移除豆包 Seedream 选项；仅一个可用模型时隐藏配图选择器
- 默认开发端口：后端 `24000`、前端 `25000`（README 与 Vite 配置同步）
- 论文卡片封面 URL 追加 `mtime` 版本参数，重生成后即时刷新

### Fixed

- 打印笔记时隐藏小节标题操作按钮，避免导出 PDF 出现多余控件

## [0.0.9] - 2026-06-09

### Added

- 笔记全文翻译：支持 `deepseek-v4-flash` 流式翻译，论文页可切换原文/译文对照
- 文献库卡片视图与列表视图：AI 一句话摘要、阅读进度、状态标签与元数据栏
- 文件夹归类：侧栏文件夹树、移动至文件夹、多文件夹归属；卡片封面配色随文件夹色系变化
- 论文卡片 AI 封面与摘要：解析完成后旁路生成；支持手动「重新生成摘要与封面」
- 笔记阅读进度：基于笔记 pane 滚动深度，localStorage 缓存 + 断点续读

### Changed

- 首页改为文献库（`LibraryPage`），统一工作区侧栏与上传入口
- 完成态「已生成笔记」移出封面，标题旁 ✓ 图标悬停提示；封面仅显示进行中状态
- 论文唯一文件夹归属变更时自动重生成摘要与封面；新增第二个文件夹不触发
- Vite dev server 允许 `.top` 域名访问
...
- 笔记流式渲染：回退 v0.0.6 中按内容长度/`len:N` 签名强制 remount XMarkdown 的逻辑，恢复 v0.0.5 增量渲染，消除最终笔记生成时在页面开头反复闪烁、无法顺序输出的问题
- 非流式增删配图仍仅按配图路径签名刷新图片缓存，不再误用全文长度作 revision



### Changed

- 笔记工具栏：生成过程中可选笔记/配图模型；下拉菜单挂载至 `document.body` 避免被面板裁剪
...
## [0.0.3](https://github.com/hk011/yanxi-paper-note/releases/tag/v0.0.3) - 2026-05-24
...
## [0.0.1](https://github.com/hk011/yanxi-paper-note/releases/tag/v0.0.1) - 2026-05-24
...
## [0.0.0](https://github.com/hk011/yanxi-paper-note/releases/tag/v0.0.0) - 2026-05-24
