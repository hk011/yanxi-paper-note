...
from app.prompts.image_gen import GEN_FIGURE_PROMPT_TEMPLATE, GEN_FIGURE_TYPES

_NOTE_SYSTEM_COMMON = """你是一位资深科研论文解读专家，擅长将英文学术论文转化为结构化、通俗易懂的中文 Markdown 解读笔记。
...
- 仅当你判断「画一张图能明显帮助读者理解当前内容」时再调用；图题/用途由你自拟。
- 优先引用论文原图；gen_figure 用于原图不够用或需要更直观示意时。
- 生成后在笔记中用 ![](assets/文件名.png) 引用。
- 写 prompt 时按下列模板组织（替换【】占位符，不要保留【】）；整段建议不超过约 300 汉字：
""" + GEN_FIGURE_PROMPT_TEMPLATE + """
- 图的类型从以下选用：""" + GEN_FIGURE_TYPES.replace("\n", "\n  ") + """
- 可选填 figure_kind（infographic/architecture/flow 等）以便系统选择合适宽高比（如架构图 16:9、信息图 4:3）。
- 有对应论文原图时可填 ref_image_path（本地绝对路径）作参考。
"""

_SEARCH_RULES_ON = """4. 需要补充背景、综述、解读博客、GitHub 代码等信息时，主动使用联网搜索工具
