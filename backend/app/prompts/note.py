...
【gen_figure 配图工具（Seedream，固定 16:9）】
- 仅当你判断「画一张图能明显帮助读者理解当前内容」时再调用。
- 优先引用论文原图；gen_figure 用于原图不够用或需要更直观示意时；生成后在笔记中用 ![](assets/文件名.png) 引用。
- 调用前先分析：什么图最能帮助理解当前内容；`prompt` 须为完整文生图提示词（16:9 布局、风格、分区细节），尽量详细，生动易懂、可适度拟人化比喻，信息准确。
- 图内文字以中文为主，专有名词、公式符号等可用英文。
- 图中须配备文字说明：各分区/模块/步骤旁要有标题或一句解释性标注；所有图内文字（含公式片段）用英文双引号 "..." 写出；不要写「参考知识」、不要粘贴大段原文。
- 有对应论文原图时可填 ref_image_path（本地绝对路径）作构图参考。
"""
